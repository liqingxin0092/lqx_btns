...
static key_unit_t key_unit[] = {
    /*key0*/
    {
        ...
        .active_state = 1,          //触发边沿设置. 1,上升沿. 0,下降沿
...
/*用户初始化函数*/
// 功能:配合你实现的hw_info_t结构体,实现初始化单个按键
void key_hw_init(hw_info_t *info)
{
    GPIO_InitTypeDef GPIO_InitStructure;
    GPIO_InitStructure.GPIO_Mode = info->Mode;
    GPIO_InitStructure.GPIO_Pin = 1<<info->GPIO_Pin;
    GPIO_Init(info->GPIOx, &GPIO_InitStructure);
}
...
{
    if((key_unit_temp->hw_info.GPIOx->IDR >> key_unit_temp->hw_info.GPIO_Pin&1) == key_unit_temp->active_state)
        return 1;
    return 0;
}
