...
static uint8_t is_KeyUnit_func(uint8_t index)
{
    ...
    {
#if DETECT_ALL_SINGLE_KEYS==1
        if (is_key_pressed(&key_unit[index]))
            return 1;
        return 0;
#else
...
/*启动计数*/
//time:单位1ms
static void key_start_counting(key_unit_t *unit_exam) {
...
static void run_one_key_timer(uint8_t index) {
    ...
    if (cur_unit->flags.timer_state==1) {
        cur_unit->timer_cnt++;
        if (cur_unit->timer_cnt==COMBO_TOLERANCE_TIME) {
            cur_unit->flags.timer_state=0; //到时/未启用
            cur_unit->timer_cnt=0;         //计数值清零
            cur_unit->combo_cnt=0;         //连击数清零
#if CATCH_HIT_STOP_EVENT==1
            /*松开事件*/
            write_circle_buffer(&key_circle_buffer, KEY_EVENT_PER_NUM*index+key0_hit_stop);//松开
#endif
